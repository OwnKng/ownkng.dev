...
We'll also need an event handler for when the cursor position leaves the plot area. This function simply sets our `tooltipTimeout` variable to a 1500ms and calls the `hideTooltip()` function.
