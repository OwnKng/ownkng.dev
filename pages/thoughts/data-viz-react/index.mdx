...
Visx isn't a charting library. It doesn't provide any template visualisations to simply supply data to. Instead, it offers a collections of geometry components, like lines, bars or arcs, that can be used to construct visualisations. Consequently, visx has a very declarative syntax - which will be familiar to any React developer.
...
The upshot of this is that visx is highly _expressive_ - allowing developers to combine different geometries and scales to create any visualisation they can imagine - while also being much easier to learn than _D3_.

# Creating a scatter plot with visx

To show this in practice, we'll create a scatter plot, based on the fantastic [Gapminder chart](https://s3-eu-west-1.amazonaws.com/static.gapminder.org/GapminderMedia/wp-uploads/20161215212516/countries_health_wealth_2016_v151.pdf) of GDP per capita and life expectancy.

The data we're going to use comes from the [WorldBank Open Data](https://data.worldbank.org/) portal. The completed visualisation and the code is available [here](https://codesandbox.io/s/objective-snow-txbsr?file=/src/App.js).

There are a few things to note about this chart before we start writing any code. First, the x axis is a logarithmic scale. This is necessary to allow the reader to discern the significant variation in GDP per capita (particularly at the lower ends of the scale), which would be lost using an arithmetic scale. For readability, the x axis labels are repeated on both the bottom and top of the chart.
...
or we can install all the visx packages by running `npm install @visx/visx`.
...
When designing a chart with visx, there are four steps we need to go through: setting the chart dimensions; creating accessor functions to select our data; creating scales to translate our data into coordinates; and then returning the visualisation.

After we've done this, we can then improve our visualisation by adding a tooltip or customising the styling.

### Step 1: Set chart dimensions

We're going to write a function component to return our scatter plot. This component will take three props to help set the dimensions of our visualisation - width, height and margin. We'll provide sensible defaults in our component.

We'll then create two new variables - `innerWidth` and `innerHeight` - which we'll use later to set the dimensions of our chart's plot area.
...
Visx provides functions to create each of these scales. We simply provide these functions with the _range_ of the scale in pixels and the data we want to scale, referred to as the _domain_. We'll also use `extent()` function from D3, which extracts the minimum and maximum value from our data.
...
The geometry we're mapping our data to are points. Visx provides a `Circle` component, which we'll need to import.
...
We can now add a map method into `<svg>` tags to return a circle for every item in our array.
...
We're still missing the legend to match the color of our points to regions. To do this, we'll first import several legend components from the `@visx/legend` library.
...
Our tooltip will rely on a [Voronoi](https://en.wikipedia.org/wiki/Voronoi_diagram) partition of the chart area, which allows us to find the closest point to hover position. A Voronoi diagram is are a way of partitioning a plane into a series of regions defined by the location of a set of points. In a Voronoi diagram, every point is surrounded by a region that covers the area of the plane that is closer to that point than any other.
...
To do this, we'll use the `localPoint()` function from the `visx/event` package, which returns an object with the coordinates of the cursor position. We'll then uses these coordinates in the `find()` method of our `voronoiLayout`. This returns the data of the closest point to the cursor position, which we provide to showTooltip function.

We'll also created a `tooltipTimeout` variable, which we'll use to display the tooltip only for a second or so after the user moves their cursor position off the chart area.

All of this is wrapped in a useCallback() hook, as the operation is computationally expensive, and so prevents unnecessary re-rendering when our `voronoiLayout` or tooltip haven't changed.
...
To add our tooltip to our chart, we need to add our `svgRef` as the ref of our `<svg>` tag
...
Below our closing `svg` tag, we now need to add the tooltip using the `<TooltipWithBounds />` component. This component also allows us to wrap any data we want to show to the user and apply any arbitrary styles.
...
We'll also amend our `<Cicle />` components by applying a highlight effect to the point that that's returned by our tooltip. We can do this by adding some logic to the `stroke` attribute.
...
This may seem like a lot of code to produce a scatter plot. However, it's important to consider that many of the features of our chart (logarithmic scales, duplicated axes, custom tooltips) aren't well supported by simple charting libraries.

The version of the chart that appears on this page also features text annotations (easily added with [@visx/text](https://airbnb.io/visx/docs/text)) and further custom styling. Again, these are features which are often difficult to achieve with other JavaScript libraries.
...
This really is where visx shines: visx is modular enough to create highly expressive data visualisations, but abstracts away much of the drudgery of things like responsiveness or interaction.
