...
<div>
  <video width='100%' autoPlay={true} controls={false} loop={true}>
...
[GRAPHIX](https://graphix-viz.vercel.app/discover) is a graph building app that allows users to import csv data, select a geometry (bar, point or line) to visualise their data and then map the variables in their data to aesthetic features - such as the x-axis, y-axis, color and labels.
...
Any csv data that a user inputs is converted to JSON using the [Papa Parse](https://www.papaparse.com/) library before it's stored in the database. The database itself is MongoDB, hosted on [MongoDB Atlas](https://www.mongodb.com/cloud/atlas). The schemas and controllers were written using [Mongoose](https://mongoosejs.com/).
...
These props are then passed to the `Discover` component, which iterates through the graphs - passing each one to a `Visualisation` component, which returns an svg graphic for each graph that's passed into it.
...
The schema for the `graphics` collection, which holds the data for each graph that a user creates, is below. Some of these fields, such as _reordered_ are only relevant for a particular geometry - in this case reordering the categories on the bar graph in descending order.
